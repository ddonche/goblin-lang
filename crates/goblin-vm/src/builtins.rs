...
///   - receive a Vec<Value> of arguments (values sit directly on the operand stack)
///   - receive &mut Session for token_store / object_store / random state access
///   - return a Value (no arena allocation — that only happens at binding points)
use std::rc::Rc;
...
use crate::value::{BuiltinId, CollectionValue, FormatSpec, GoblinDateKind, GoblinDateTime, Value};
...
fn dispatch(id: BuiltinId, args: Vec<Value>, session: &mut Session) -> Result<Value, GoblinError> {
    ...
    match id {
        // ── Memory ───────────────────────────────────────────────────────────
        BuiltinId::MemId => {
            // mem_id requires a slot-direct opcode (MemId(slot)) to obtain the arena address;
            // since the stack now holds Values (not Tethers), the real address is unavailable here.
            Err(GoblinError::Runtime(
                "mem_id: arena introspection is not supported in this context (stack holds values, not tethers)".into()
            ))
        }
        BuiltinId::MemAddr => {
            // Same limitation as MemId.
            Err(GoblinError::Runtime(
                "mem_addr: arena introspection is not supported in this context (stack holds values, not tethers)".into()
            ))
        }
        BuiltinId::MemTotal => {
        ...
        BuiltinId::Map => {
            ...
            match read(0)? {
                Value::Str(s) => {
                    ...
                    let mut results = vec![];
                    for c in s.chars() {
                        let v = call_builtin(bid, vec![Value::Char(c)], session)?;
                ...
                Value::Array(xs) => {
                    let mut out = vec![];
                    for v in xs {
                        let result = call_builtin(bid, vec![v], session)?;
                        out.push(result);
                    }
                    Ok(Value::Array(out))
                }
        ...
        BuiltinId::RollStr => {
            ...
            dispatch(BuiltinId::Roll, vec![Value::Map(cfg)], session)
        }
        BuiltinId::RollDetailStr => {
            ...
            let cfg = parse_dice_string_to_map(&dice_str)?;
            dispatch(BuiltinId::RollDetail, vec![Value::Map(cfg)], session)
        }
